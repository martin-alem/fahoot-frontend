...
const Library: React.FC = () => {
  ...
  const [
    createQuiz,
    {
      ...
      isLoading: isLoadingCreateQuiz,
      isSuccess: isSuccessCreateQuiz,
      isError: isErrorCreateQuiz,
    },
  ...
  useEffect(() => {
    if (isErrorGetQuizzes) {
      const statusCode =
        errorGetQuizzes && 'status' in errorGetQuizzes ? errorGetQuizzes.status : 500;
  ...
  useEffect(() => {
    if (isErrorCreateQuiz) {
      const statusCode =
        errorCreateQuiz && 'status' in errorCreateQuiz ? errorCreateQuiz.status : 500;
      const errorMessage = serverErrors(statusCode);
      toast.error(errorMessage, { position: toast.POSITION.TOP_CENTER });
    }
  }, [isErrorCreateQuiz]);
        ...
        <div className="mx-auto flex justify-between items-center max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div>
            <Button
              ...
              loading={isLoadingCreateQuiz}
              type="primary"
              label="Create"
              disabled={isLoadingCreateQuiz}
              prefixIcon={<PencilIcon className="w-6" />}
            />
          </div>
        </div>
      </header>
      <main>
        <div className="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
          ...
          {isLoadingGetQuizzes || isFetchingGetQuizzes ? (
            <Shimmer count={quizzes?.results.length ?? PAGE_SIZE} />
          ) : (
            <>
                ...
                <div className="w-full p-2 md:p-0">
                  ...
                  <div className="hidden md:block">
                    {view === 'grid' ? (
                      ...
                      <QuizListContainer quizzes={quizzes.results} />
                    )}
                  </div>
                </div>
              ) : (
                <EmptyState
                  icon={<DocumentPlusIcon className="w-12 text-primary-500" />}
                  heading="No quizzes found"
                  description="Get started by creating a new quiz"
          ...
          {quizzes?.results && quizzes.results.length > 0 && (
            <div className="w-full mt-6 flex items-center">
              <Pagination
                ...
                currentNumberOfResults={quizzes.results.length}
                page={page}
                setPage={setPage}
              />
            </div>
